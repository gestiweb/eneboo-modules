...
class oficial extends interna {
	...
	function redondearEjeYLineal(valor:Number):Number {
		return this.ctx.oficial_redondearEjeYLineal(valor);
	}
...
{
	var f:Object = new FLFormSearchDB("gf_lineal");
...
debug("XXXXXXXXXXX\noficial_dibujarGrafico: " + xmlDatos.toString(4));
	...
	var acumularValores:Boolean = (eGrafico.attribute("AcumularValores") == "true");
...
debug("maxY funcion = " + maxY);
		...
		if (!marcarCadaY || marcarCadaY == 0) {
			marcarCadaY = this.iface.redondearEjeYLineal(maxY / 5);
...
debug(alto + " -- " + margenSuperior + " -- " + altoTitulo + " -- " + altoMedidaY + " -- " + margenInferior + " -- " + margenLabelsX + " -- " + altoLeyenda + " -- " + filasLeyenda);
	
	var factorX:Number = anchoNeto / (maxX - minX-1);
	var factorY:Number = altoNeto / (maxY - minY);
	
	var fMaxX = ((maxX - minX-1) * factorX);
	...
	if (!isNaN(marcarCadaX) && marcarCadaX > 0) {
		...
		for (var x:Number = minX; x < maxX; x += marcarCadaX) {
			...
			if (x < maxX) {
				...
				if (anguloLabelX != 0) {
					pic.savePainter();
// 					pic.translate(parseInt(offGraficoX) + parseInt(fX) + factorX, parseInt(offGraficoY) + parseInt(fMaxY));
					pic.translate(parseInt(offGraficoX) + parseInt(fX), parseInt(offGraficoY) + parseInt(fMaxY));
...
/// Lineas
	var anchoSecuencia:Number = factorX * 95 / 100;
	var colorBarras:String;
	var iSecuencia:Number = 0;
	debug(xmlSecuenciasValor.toString(4));
	var x:Number;
	var y:Number
	var xAnt:Number;
	var yAnt_Number;
	...
	for (iSecuencia = 0; iSecuencia < numSecuencias; iSecuencia++) {
		...
		for (var i:Number = 0; i < nodos.length(); i++) {
			...
			if (acumularValores) {
				valorY += parseFloat(valor.attribute("Y"));
			} else {
				valorY = parseFloat(valor.attribute("Y"));
			}
			x = (parseFloat(offGraficoX) + (parseFloat(valor.attribute("X")) * parseFloat(factorX))) - (minX * factorX);
			y = ((parseFloat(fMaxY) - (valorY * parseFloat(factorY))) + parseFloat(offGraficoY)) + (minY * factorY);
...
function oficial_restaurarValoresDefecto():Boolean
{
	...
	xmlDoc.setContent("<Grafico Tipo='lineal' />");
	...
	var eEjeX:FLDomElement = xmlDoc.createElement("EjeX");
...
// 	eEjeX.setAttribute("MarcarLabels", "true");
	...
	var eEjeY:FLDomElement = xmlDoc.createElement("EjeY");
	...
	var eSecuencias:FLDomElement = xmlDoc.createElement("Secuencias");
...
// 			eValor.setAttribute("LabelX", aValores[s][i][2]);
		}
	}
...
function oficial_cargarCampos():Boolean
{
	...
	this.child("fdbAcumularValores").setValue(eGrafico.attribute("AcumularValores") == "true");
...
// 	this.child("fdbMarcarLabelX").setValue(eEjeX.attribute("MarcarLabels") == "true");
	...
	var eEjeY:FLDomElement = eGrafico.namedItem("EjeY").toElement();
	this.child("fdbMinY").setValue(parseFloat(eEjeY.attribute("Min")));
...
function oficial_refrescarGrafico()
{
	...
	eGrafico.setAttribute("AcumularValores", (cursor.valueBuffer("acumularvalores") ? "true" : "false"));
...
\end */
function oficial_redondearEjeYLineal(valor:Number):Number
{
	var v:Number = valor;
	var pot:Number = 0;
	while (v > 10) {
...
function oficial_calcularMaxY(xmlDatos:FLDomDocument):Number
{
	...
	for (var s:Number = 0; s < xmlSecuencias.length(); s++) {
		...
		for (var v:Number = 0; v < xmlValores.length(); v++) {
			...
			if (acumularValores) {
				totalSecuencia += valor;
			} else {
				max = (valor > max ? valor : max);
			}
		}
		if (acumularValores) {
			max = (totalSecuencia > max ? totalSecuencia : max);
		}
	}
	return max;
}
...
// 	if (!util.sqlSelect("gf_lineal", "id", "id = " + id) && cursor.modeAccess() != cursor.Insert) {
