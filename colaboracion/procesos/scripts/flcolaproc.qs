...
class oficial extends interna {
	...
	function iniciarTarea(curTareas:FLSqlCursor, idUser:String) {
		return this.ctx.oficial_iniciarTarea(curTareas, idUser);
	}
...
class infosial extends oficial {
	...
	function descripcionTarea(curTarea:FLSqlCursor):String {
		return this.ctx.infosial_descripcionTarea(curTarea);
	}
	function estadoObjetoInicial(idProceso:Number):String {
		return this.ctx.infosial_estadoObjetoInicial(idProceso);
	}
	function asignarTareas(idProceso:String, idTipoProceso:String):Boolean {
		return this.ctx.infosial_asignarTareas(idProceso, idTipoProceso);
	}
	function terminarTareaEsp(curTarea:FLSqlCursor):Boolean {
		return this.ctx.infosial_terminarTareaEsp(curTarea);
	}
	function tbnIniciarTareaSClicked(idUsuario:String):Boolean {
		return this.ctx.infosial_tbnIniciarTareaSClicked(idUsuario);
	}
	function resolverIncidencia():Boolean {
		return this.ctx.infosial_resolverIncidencia();
	}
	function despertarTareaEsp(curTarea:FLSqlCursor):Boolean {
		return this.ctx.infosial_despertarTareaEsp(curTarea);
	}
	function condicionSecuenciaOK(idProceso:String, secuencia:String):Boolean {
		return this.ctx.infosial_condicionSecuenciaOK(idProceso, secuencia);
	}
}
//// INFOSIAL ///////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
...
class ifaceCtx extends head {
	...
	function pub_iniciarTarea(curTareas:FLSqlCursor, idUser:String):Boolean {
		return this.iniciarTarea(curTareas, idUser);
	}
...
\end */
function oficial_iniciarTarea(curTareas:FLSqlCursor, idUser:String)
{
	var util:FLUtil = new FLUtil();
...
@return	Descripción
\end */
function infosial_descripcionTarea(curTarea:FLSqlCursor):String
{
debug("infosial_descripcionTarea");
	var util:FLUtil = new FLUtil;
	var desTarea:String;
	var codTipoTareaPro:String = curTarea.valueBuffer("codtipotareapro");
debug("codTipoTareaPro = " + codTipoTareaPro);
	switch (codTipoTareaPro) {
		case "SC_INCIDENCIA_SC_RESOLVER": {
debug("SC_INCIDENCIA_SC_RESOLVER");
			var qryIncidencia:FLSqlQuery = new FLSqlQuery();
			qryIncidencia.setTablesList("se_incidencias,clientes");
			qryIncidencia.setSelect("c.nombre, i.desccorta");
			qryIncidencia.setFrom("se_incidencias i LEFT OUTER JOIN  clientes c ON c.codcliente = i.codcliente");
			qryIncidencia.setWhere("i.codigo = '" + curTarea.valueBuffer("idobjeto") + "'");
			qryIncidencia.setForwardOnly(true);
			if (!qryIncidencia.exec()) {
				return false;
			}
debug("consulta " + qryIncidencia.sql());
			if (!qryIncidencia.first()) {
				desTarea = this.iface.__descripcionTarea(curTarea);
			}
			var cliente:String = qryIncidencia.value("c.nombre");
			if (cliente && cliente != "" && cliente.length > 20) {
				cliente = cliente.left(20) + "...";
			}
			desTarea = cliente + " - " + qryIncidencia.value("i.desccorta");
			break;
		}
		default: {
			desTarea = this.iface.__descripcionTarea(curTarea);
...
function infosial_asignarTareas(idProceso:String, idTipoProceso:String):Boolean
{
	var util:FLUtil = new FLUtil;

	switch (idTipoProceso) {
		case "SC_INCIDENCIA": {
			var codIncidencia:String = util.sqlSelect("pr_procesos", "idobjeto", "idproceso = " + idProceso);
			var datosIncidencia:Array = flfactppal.iface.pub_ejecutarQry("pr_procesos p INNER JOIN se_incidencias i ON p.idobjeto = i.codigo", "i.codencargado", "p.idproceso = " + idProceso, "se_incidencias,pr_procesos");
			if (datosIncidencia.result != 1) {
				return this.iface.__asignarTareas(idProceso, idTipoProceso);
			}
			if (!util.sqlUpdate("pr_tareas", "iduser", datosIncidencia["i.codencargado"], "idproceso = " + idProceso)) {
				return false;
			}
			break;
		}
...
function infosial_tbnIniciarTareaSClicked(idUsuario:String):Boolean
{
	var util:FLUtil = new FLUtil;
	var ok:Boolean = true;
	if (!this.iface.cursor_.isValid()) {
	...
	var estado:String = this.iface.cursor_.valueBuffer("estado");
	if (estado == "PTE") {
		return this.iface.__tbnIniciarTareaSClicked(idUsuario);
	}

	var codTipoTareaPro:String = this.iface.cursor_.valueBuffer("codtipotareapro");
	switch (codTipoTareaPro) {
		case "SC_INCIDENCIA_SC_RESOLVER": {
			var codIncidencia:String = this.iface.cursor_.valueBuffer("idobjeto");
			if (this.iface.curObjeto_) {
				delete this.iface.curObjeto_;
			}
			this.iface.curObjeto_ = new FLSqlCursor("se_incidencias");
			this.iface.curObjeto_.select("codigo = '" + codIncidencia + "'");
			if (!this.iface.curObjeto_.first()) {
				return false;
			}
			connect(this.iface.curObjeto_, "bufferCommited()", this, "iface.refrescarTareasS");
...
function infosial_resolverIncidencia():Boolean
{
	var idProceso:String = this.iface.cursor_.valueBuffer("idproceso");
	if (!this.iface.establecerAtributoProceso(idProceso, "EstadoIncidencia", "Resuelta")) {
		return false;
	}
	if (!this.iface.__tbnIniciarTareaSClicked()) {
		return false;
	}
	return true;
}

function infosial_terminarTareaEsp(curTarea:FLSqlCursor):Boolean
{
debug("Terminando " + curTarea.valueBuffer("codtipotareapro"));
	var util:FLUtil = new FLUtil;
	switch (curTarea.valueBuffer("codtipotareapro")) {
		case "SC_INCIDENCIA_SC_RESOLVER2": {
			var idProceso:String = curTarea.valueBuffer("idproceso");
			var nodoProceso:FLDomNode = this.iface.dameXMLProceso(idProceso);
			var estadoIncidencia:String = nodoProceso.toElement().attribute("EstadoIncidencia");
			var hoy:Date = new Date;
			if (estadoIncidencia && estadoIncidencia != "") {
				var codIncidencia:String = curTarea.valueBuffer("idobjeto");
				...
				curIncidencia.setValueBuffer("estado", estadoIncidencia);
				if (estadoIncidencia == "Resuelta") {
					curIncidencia.setValueBuffer("fechacierre", hoy.toString());
				}
...
// 			if (codigoTT && codigoTT != "") {
				...
				eProceso = xmlProceso.firstChild().toElement();
				///
				var res:Number = MessageBox.information(util.translate("scripts", "¿finalizar?"), MessageBox.Yes, MessageBox.No);
				var fin:String = (res == MessageBox.Yes ? "true" : "false");
				eProceso.setAttribute("Finalizar", fin);
				///
				curProceso.setValueBuffer("xmlparametros", xmlProceso.toString(4));
				if (!curProceso.commitBuffer()) {
					return false;
...
// 			}
		...
		default: {
			if (!this.iface.__terminarTareaEsp(curTarea)) {
				return false;
			}
		}
	}
...
function infosial_estadoObjetoInicial(idProceso:Number):String
{
	var util:FLUtil = new FLUtil;
	var estado:String;
	if (util.sqlSelect("pr_procesos", "idproceso" , "idproceso = " + idProceso + " AND idtipoproceso = 'SC_INCIDENCIA'")) {
		estado = "";
	} else {
		estado = this.iface.__estadoObjetoInicial(idProceso);
	}
	return estado;
}

function infosial_despertarTareaEsp(curTarea:FLSqlCursor):Boolean
{
	var codTipoTareaPro:String = curTarea.valueBuffer("codtipotareapro");
	switch (codTipoTareaPro) {
		case "SC_INCIDENCIA_SC_RESOLVER": {
			var estadoTarea:String = curTarea.valueBuffer("estado");
			...
			curIncidencia.select("codigo = '" + curTarea.valueBuffer("idobjeto") + "'");
			if (!curIncidencia.first()) {
				return false;
			}
			curIncidencia.setModeAccess(curIncidencia.Edit);
			curIncidencia.refreshBuffer();
			curIncidencia.setValueBuffer("estado", estadoIncidencia);
			curIncidencia.setNull("fecharevision");
			if (!curIncidencia.commitBuffer()) {
				return false;
			}
			break;
...
function infosial_condicionSecuenciaOK(idProceso:String, secuencia:String):Boolean
{
debug("Evaluando secuencia " + secuencia);
	var util:FLUtil = new FLUtil;
	var nodoProceso:FLDomNode;
	var eProceso:FLDomElement = false;
	var ok:Boolean;

	switch (secuencia) {
		case "BUCLE_TAREA_2 >> BUCLE_TAREA": {
			nodoProceso = this.iface.dameXMLProceso(idProceso);
			if (!nodoProceso) {
				MessageBox.warning(util.translate("scripts", "Error al evaluar la condición para la secuencia %1 del proceso %2").arg(secuencia).arg(idProceso), MessageBox.Ok, MessageBox.NoButton);
				return false;
			}
			eProceso = nodoProceso.toElement();
			ok = (eProceso.attribute("Finalizar") == "false" ? true : false);
			break;
		}
		case "BUCLE_TAREA_2 >> BUCLE_TAREA_3": {
			nodoProceso = this.iface.dameXMLProceso(idProceso);
			if (!nodoProceso) {
				MessageBox.warning(util.translate("scripts", "Error al evaluar la condición para la secuencia %1 del proceso %2").arg(secuencia).arg(idProceso), MessageBox.Ok, MessageBox.NoButton);
				return false;
			...
			eProceso = nodoProceso.toElement();
			ok = (eProceso.attribute("Finalizar") == "true" ? true : false);
			break;
		}
		default: {
			ok = this.iface.__condicionSecuenciaOK(idProceso, secuencia);
		}
	}
debug("Evaluacion = " + ok);
	return ok;
