...
*/
	...
	function consultarStock(referencia:String):Boolean {
		return this.ctx.oficial_consultarStock(referencia);
	}
	function dameRefConsultaStock():String {
		return this.ctx.oficial_dameRefConsultaStock();
	}
}
...
class ifaceCtx extends head {
	...
	function pub_consultarStock(referencia:String):Boolean {
		return this.consultarStock(referencia);
	}
	function pub_dameRefConsultaStock():String {
		return this.dameRefConsultaStock();
	}
}
...
function oficial_crearFactura(curComanda:FLSqlCursor):Number
{
	...
	if (!this.iface.totalesFactura(curComanda)) {
		return false;
	}
	if (!this.iface.curFactura.commitBuffer()) {
		return false;
...
\end */
function oficial_totalesFactura(curComanda:FLSqlCursor):Boolean
{
		...
		case curComanda.Del: {
			...
			while (curPago.next()) {
				if (!curPago.valueBuffer("editable")) {
					MessageBox.warning(util.translate("scripts", "El pago de la venta está bloqueado, posiblemente por estar cerrado su arqueo.\nNo es posible eliminarlo."), MessageBox.Ok, MessageBox.NoButton);
					return false;
				}
...
function oficial_consultarStock(referencia:String):Boolean
{
	var util:FLUtil = new FLUtil();
	var f:Object = new FLFormSearchDB("tpv_consultastocks");

	f.setMainWidget();
	this.iface.refConsultaStock_ = referencia;
	var idStock:String = f.exec("idstock");
}

function oficial_dameRefConsultaStock():String
{
	return this.iface.refConsultaStock_;
}
